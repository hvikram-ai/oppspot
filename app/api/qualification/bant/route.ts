...
export async function POST(request: NextRequest) {
  try {
    ...
    const body: CalculateBANTRequest = await request.json();
    ...
    if (!qualification) {
      return NextResponse.json(
        { error: 'Failed to calculate BANT qualification' },
        { status: 500 }
      );
    }
    ...
    console.error('Error in BANT API:', error);
    return NextResponse.json(
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get existing BANT qualification
    ...
    if (error && error.code !== 'PGRST116') { // Not found is ok
      return NextResponse.json(
        { error: 'Failed to fetch BANT qualification' },
        { status: 500 }
      );
    }
    ...
    console.error('Error in BANT GET API:', error);
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    );
