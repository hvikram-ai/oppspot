...
CREATE INDEX idx_predictive_scores_company ON predictive_scores(company_id);
CREATE INDEX idx_predictive_scores_org ON predictive_scores(org_id, prediction_date DESC);
CREATE INDEX idx_predictive_scores_probability ON predictive_scores(buying_probability DESC);
CREATE INDEX idx_predictive_scores_timeline ON predictive_scores(predicted_timeline_days);
CREATE INDEX idx_predictive_scores_priority ON predictive_scores(priority_score DESC);

CREATE INDEX idx_prediction_history_prediction ON prediction_history(prediction_id, created_at DESC);
CREATE INDEX idx_prediction_history_company ON prediction_history(company_id, created_at DESC);

CREATE INDEX idx_signal_aggregates_company ON signal_aggregates(company_id);
CREATE INDEX idx_signal_aggregates_velocity ON signal_aggregates(signal_velocity_30d DESC);

CREATE INDEX idx_prediction_alerts_org ON prediction_alerts(org_id, created_at DESC);
CREATE INDEX idx_prediction_alerts_priority ON prediction_alerts(alert_priority, dismissed);
...
-- Predictive Scores
CREATE POLICY "Users view their org's predictions" ON predictive_scores
  FOR SELECT USING (
...
-- Prediction History
CREATE POLICY "Users view their org's prediction history" ON prediction_history
  FOR SELECT USING (
    company_id IN (
      SELECT company_id FROM predictive_scores
...
-- Prediction Alerts
CREATE POLICY "Users view their org's alerts" ON prediction_alerts
  FOR SELECT USING (
    org_id IN (SELECT org_id FROM profiles WHERE id = auth.uid())
  );
...
-- 9. Create trigger for prediction history
...
-- 10. Create function to refresh signal aggregates
CREATE OR REPLACE FUNCTION refresh_signal_aggregates(p_company_id UUID)
RETURNS void AS $$
DECLARE
  v_signals_7d INTEGER;
  v_signals_30d INTEGER;
  v_signals_60d INTEGER;
  v_signals_90d INTEGER;
  v_funding INTEGER;
  v_hiring INTEGER;
  v_technology INTEGER;
  v_expansion INTEGER;
  v_executive INTEGER;
  v_financial INTEGER;
BEGIN
