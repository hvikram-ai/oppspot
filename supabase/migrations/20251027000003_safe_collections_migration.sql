-- Safe Collections Migration
-- This handles partial migrations by using CREATE IF NOT EXISTS and DROP IF EXISTS
-- Run this if you got "type already exists" errors

-- ============================================================================
-- STEP 1: Drop existing ENUMs if they exist (to start fresh)
-- ============================================================================
...
-- ============================================================================
-- STEP 3: Create Tables (drop first if exists)
-- ============================================================================
...
CREATE INDEX IF NOT EXISTS idx_collection_items_chronological
ON collection_items(collection_id, added_at DESC);

-- Index for polymorphic reference lookups
CREATE INDEX IF NOT EXISTS idx_collection_items_polymorphic
ON collection_items(item_type, item_id);

-- Index for permission checks
CREATE INDEX IF NOT EXISTS idx_collection_access_lookup
ON collection_access(user_id, collection_id);

-- Index for stream ownership queries
CREATE INDEX IF NOT EXISTS idx_collection_access_stream
...
CREATE INDEX IF NOT EXISTS idx_collections_system
ON collections(user_id, is_system)
WHERE is_system = TRUE;

-- Index for active stream lookups in profiles
CREATE INDEX IF NOT EXISTS idx_profiles_active_stream
ON profiles(active_collection_id);

-- Unique index to ensure one General stream per user
DROP INDEX IF EXISTS idx_collections_one_general_per_user;
CREATE UNIQUE INDEX idx_collections_one_general_per_user
ON collections(user_id, is_system)
WHERE is_system = TRUE;

-- ============================================================================
-- STEP 6: Create Trigger Function for Auto-Creating General Stream
-- ============================================================================
