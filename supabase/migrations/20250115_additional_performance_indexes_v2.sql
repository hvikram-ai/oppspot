-- Additional Performance Indexes (V2 - Non-Concurrent)
-- Indexes for newer features (ResearchGPT, ITP, Weekly Updates, Data Room Q&A)
-- Date: 2025-01-15
-- NOTE: Using regular CREATE INDEX (not CONCURRENTLY) for compatibility with transactions
...
CREATE INDEX IF NOT EXISTS idx_itps_org_active
...
CREATE INDEX IF NOT EXISTS idx_itp_matches_created
...
CREATE INDEX IF NOT EXISTS idx_document_chunks_data_room
...
CREATE INDEX IF NOT EXISTS idx_qa_feedback_helpful
...
BEGIN
  ...
  RAISE NOTICE '';
  RAISE NOTICE 'Fixed in v2: Corrected enum values (complete/generating instead of completed/in_progress)';
END $$;
