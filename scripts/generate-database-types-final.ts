 ...
 * Final version: Fixed array type detection and better column parsing
 */
...
function sqlToTs(sqlType: string): string {
  const type = sqlType.toUpperCase().trim().replace(/\s+/g, ' ');

  // Check for array types first (TEXT[], UUID[], etc.)
  if (type.match(/\[\s*\]$/)) {
    const baseType = sqlToTs(type.replace(/\[\s*\]$/, ''));
    return `${baseType}[]`;
  }
...
function parseCreateTable(sql: string): Table | null {
  ...
  // Split by comma at the root level only
  const parts: string[] = [];
  let current = '';
  let depth = 0;
  ...
  for (const part of parts) {
    const cleaned = removeComments(part).trim();
    if (!cleaned) continue;

    // Skip constraints
    if (cleaned.match(/^(CONSTRAINT|PRIMARY\s+KEY\s+\(|FOREIGN\s+KEY|UNIQUE\s*\(|CHECK\s*\()/i)) {
      continue;
    }

    // Parse column: name TYPE [constraints]
    // Handle types like TEXT[], UUID[], JSONB, VARCHAR(255), etc.
    const colMatch = cleaned.match(/^([a-zA-Z0-9_]+)\s+([A-Z0-9_]+(?:\([^)]+\))?(?:\s*\[\s*\])?)/i);
    if (colMatch) {
      ...
      if (cleaned.match(/\[\s*\]/)) {
        colType = colType.replace(/\s*\[\s*\]/, '') + '[]';
      }

      // Check for NOT NULL
      const nullable = !cleaned.toUpperCase().includes('NOT NULL');

      // Check for DEFAULT or PRIMARY KEY (has default)
      const hasDefault = cleaned.toUpperCase().includes('DEFAULT') ||
                        cleaned.toUpperCase().includes('PRIMARY KEY');
